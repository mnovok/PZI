...
function handleAddCardClick(e){
    ...
    const cardElement = document.importNode(cardTemplate.content, true); //stvori element
    ...
    cardsContainer.appendChild(cardElement); //dodaj
}
...
document.getElementById('search-box').addEventListener('keyup',(e)=>{
    ...
    for (let index = 0; index < cards.length; index++) {
        ...
        if(card.textContent.indexOf(value) >= 0) //postoji li indeks toga sto smo upisali
        {
        ...
        {
            card.classList.add('hidden'); //display: none nije isto kao visibility: none
        }
    }
    //cardElement.textContent.indexOf(nekiString) == -1 //ne nalazi se
    //cardElement.textContent.indexOf(nekiString) >= 0 //nalazi se
...
function handleStarIconClick(e){
    ...
    let flag = false; //odrediti jesmo li trenutno prilikom iteriranja prosli kliknutu zvjezdicu

    for (let index = 0; index < starSiblings.length; index++) {
        const currentStar = starSiblings[index];


        if(!flag) //zeljeni se nalazi lijevo od
        {
